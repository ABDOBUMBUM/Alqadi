...
import {
  ...
  MapPin, Calendar, DollarSign, ArrowLeft, HelpCircle,
} from "lucide-react";
...
export default function ManpowerPage() {
  ...
  const [heroTitle, setHeroTitle] = useState("خدمات الأيادي العاملة من القاضي");
  const [heroDesc, setHeroDesc] = useState("نربط في مجموعة القاضي الذهبية أصحاب العمل بالكفاءات المؤهلة في جميع القطاعات، مع متابعة شاملة من التوظيف حتى ما بعد التعاقد لضمان أعلى معايير الجودة.");
  const [cmsManpower, setCmsManpower] = useState<any>({});
  const [cmsCategories, setCmsCategories] = useState<string[]>(categories);
  const [cmsProcess, setCmsProcess] = useState<any[]>(process);
  const [cmsEmployerFeatures, setCmsEmployerFeatures] = useState<string[]>([
  ...
  const statIcons = [Briefcase, Building2, Globe2, Star];
  ...
  useEffect(() => {
    fetch("/api/content")
      ...
      .then((data) => {
        ...
        if (data.cms_manpower?.heroTitle) setHeroTitle(data.cms_manpower.heroTitle);
        if (data.cms_manpower?.heroDesc) setHeroDesc(data.cms_manpower.heroDesc);
        if (Array.isArray(data.cms_manpower?.categories) && data.cms_manpower.categories.length > 0) {
          setCmsCategories(data.cms_manpower.categories);
        }
        if (Array.isArray(data.cms_manpower?.process) && data.cms_manpower.process.length > 0) {
          setCmsProcess(data.cms_manpower.process);
        }
        if (Array.isArray(data.cms_manpower?.employerFeatures) && data.cms_manpower.employerFeatures.length > 0) {
          setCmsEmployerFeatures(data.cms_manpower.employerFeatures);
        }
        if (Array.isArray(data.cms_manpower?.stats) && data.cms_manpower.stats.length > 0) {
          const mappedStats = data.cms_manpower.stats.map((s: any, idx: number) => ({
            ...s,
            icon: ICON_MAP[s.icon] || statIcons[idx % statIcons.length] || HelpCircle
          }));
          setStats(mappedStats);
        }
        if (data.cms_manpower) setCmsManpower(data.cms_manpower);
      })
      .catch(() => {});
  }, []);
  ...
  const filteredJobs = activeCategory === "الكل" 
    ? apiJobs 
    : apiJobs.filter(job => job.category === activeCategory);
    ...
    <div className="relative min-h-screen pt-28 marble-bg" style={{ color: "var(--page-text)" }}>
      ...
      <section className="relative overflow-hidden py-20 text-center">
        ...
        <motion.div {...reveal} className="relative z-10 mx-auto max-w-4xl px-6">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <a
              href={`https://api.whatsapp.com/send?phone=${waPhone}&text=${encodeURIComponent("مرحباً، أود الاستفسار عن خدمات التوظيف")}`}
              target="_blank" rel="noopener noreferrer"
              className="btn-gold gap-2"
            >
              <MessageCircle className="h-4 w-4" />
              {cmsManpower?.ctaBtnCv || "أرسل سيرتك الذاتية"}
            </a>
            <a href={`tel:+${waPhone}`} className="btn-ghost-gold gap-2">
              <Phone className="h-4 w-4" />
              {cmsManpower?.ctaBtnContact || "تواصل معنا"}
            </a>
          </div>
        </motion.div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 md:px-10">
        <div className="grid grid-cols-2 gap-5 md:grid-cols-4">
          {stats.map((s: any, i) => {
            const StatIcon = s.icon || statIcons[i % statIcons.length] || HelpCircle;
            return (
            ...
            >
              <StatIcon className="mx-auto mb-3 h-7 w-7 text-gold-400" />
        ...
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          <motion.div {...reveal} className="mb-14 text-center">
            ...
            <h2 className="mt-4 text-3xl font-bold" style={{ color: "var(--page-text)" }}>
              {cmsManpower?.processTitle || "كيف تعمل خدمة مجموعة القاضي للتوظيف؟"}
            </h2>
          </motion.div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10">
        <motion.div {...reveal} className="mb-8">
          ...
          <h2 className="mt-4 text-3xl font-bold" style={{ color: "var(--page-text)" }}>
            {cmsManpower?.jobsTitle || "الوظائف المتاحة الآن"}
          </h2>
          <p className="mt-2" style={{ color: "var(--page-text-muted)" }}>
            {cmsManpower?.jobsDesc || "فرص عمل حقيقية في الكويت والخليج العربي"}
          </p>
        </motion.div>
          ...
          {cmsCategories.map((cat) => (
            <button key={cat}
              ...
              className={`rounded-full border px-4 py-1.5 text-xs transition ${
                cat === activeCategory
                  ? "border-gold-500/60 bg-gold-500/15 text-gold-300 font-bold"
                  : "hover:border-gold-500/30 hover:text-gold-300"
              }`}
              style={cat === activeCategory ? undefined : { borderColor: "var(--page-border-subtle)", background: "var(--page-surface)", color: "var(--page-text-muted)" }}
            >
              {cat}
            </button>
          ))}
            ...
            {filteredJobs.map((job, i) => (
              ...
              >
                ...
                <div className="mt-4 flex gap-3">
                  ...
                  <a
                    href={`https://api.whatsapp.com/send?phone=${waPhone}&text=${encodeURIComponent(`مرحباً، أود التقدم لوظيفة ${job.title}`)}`}
                    target="_blank" rel="noopener noreferrer"
                  ...
                  >
                    ...
                    {cmsManpower?.applyBtnText || "تقدّم الآن"}
                  </a>
                </div>
              </motion.article>
            ))}
          ...
          <div className="grid items-center gap-12 md:grid-cols-2">
            <motion.div {...reveal}>
              ...
              <h2 className="mt-4 text-3xl font-bold md:text-4xl" style={{ color: "var(--page-text)" }}>
                {cmsManpower?.employersTitle || "أنت صاحب عمل؟ مجموعة القاضي هنا لخدمتك"}
              </h2>
              <p className="mt-4 leading-relaxed" style={{ color: "var(--page-text-muted)" }}>
                {cmsManpower?.employersDesc || "نساعدك في مجموعة القاضي في إيجاد الكفاءات المناسبة بسرعة وموثوقية. قدّم متطلباتك وسيتولى فريقنا الاستقطاب والتصفية الأولية بكل احترافية."}
              </p>
              <ul className="mt-6 space-y-3">
                {cmsEmployerFeatures.map(feat => (
              ...
              <a
                href={`https://api.whatsapp.com/send?phone=${waPhone}&text=${encodeURIComponent("مرحباً، نحن شركة ونرغب في التوظيف عبر مجموعة القاضي")}`}
                target="_blank" rel="noopener noreferrer"
                className="btn-gold mt-8 inline-flex gap-2"
              >
                <Building2 className="h-4 w-4" />
                {cmsManpower?.employersBtn || "تواصل كصاحب عمل"}
              </a>
            </motion.div>
      ...
      <section className="mx-auto max-w-4xl px-6 py-20 text-center">
        <motion.div {...reveal} className="rounded-3xl border border-gold-500/25 bg-gradient-to-r from-gold-500/10 to-transparent p-10">
          <h2 className="text-2xl font-bold" style={{ color: "var(--page-text)" }}>
            {cmsManpower?.ctaTitle || "هل أنت جاهز للخطوة القادمة مع مجموعة القاضي؟"}
          </h2>
          <p className="mt-3" style={{ color: "var(--page-text-muted)" }}>
            {cmsManpower?.ctaDesc || "أرسل سيرتك الذاتية الآن وسيتواصل معك فريق توظيف مجموعة القاضي خلال 24 ساعة."}
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <a
              href={`https://api.whatsapp.com/send?phone=${waPhone}&text=${encodeURIComponent("مرحباً، أود إرسال سيرتي الذاتية")}`}
              target="_blank" rel="noopener noreferrer"
              className="btn-gold gap-2"
            >
              <MessageCircle className="h-4 w-4" />
              {cmsManpower?.ctaBtn || "أرسل CV عبر واتساب"}
            </a>
